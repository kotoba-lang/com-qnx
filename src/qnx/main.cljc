(ns qnx.main
  "Kotodama WASM entrypoint for the QNX clean-room actor (L4) — Clojure port.
  ...
  facts (`qnx.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"qnx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "qnx-compat" :tier tier :entities entities} 200])
